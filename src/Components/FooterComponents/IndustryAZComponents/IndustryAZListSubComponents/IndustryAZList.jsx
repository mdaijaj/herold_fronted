...
import { IndustryAZListMain } from "./IndustryAZListComponents/IndustryAZListMain";
import { FooterSection } from "../../../FooterSection";
import { NavigationBar } from "../../../NavigationBar";
export const IndustryAZList = () => {
    const { setZipCodeList, setSpecificIndustryList, setRandomIndustries, setCompanyList, setGradimoAdviceNow, setSugCityList, setSugDistrictList, setIndustryAZList, setSpecificIndustryCityList, setSpecificIndustryCompanyList, setCityContent, productIdList, setCityList, setProductsList, counselerList, setCounselerIdList, counselerIdList, baseBackendRoute, setProductIdList, productsList, setCounselerList } = useContext(AppContext);
    ...
    useEffect(() => {
        ...
        const getAllSpecificCompanies = async () => {
            await axios.get(`${baseBackendRoute}/api/v1/companies/get-company-base-on-industry/${encodeURIComponent(industry)}/`)
                .then(async (res) => {
                    console.log(res);
                    const tp = await res.data;
                    setSpecificIndustryList(tp);
                })
                .catch((err) => {
                    console.log(err);
                })
