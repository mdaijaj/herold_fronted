(function ($, root, undefined) {
  $(function () {
    ...
    $(".decision a").on("click", function (e) {
      e.preventDefault();
      $("html, body").animate(
        { scrollTop: $(".cardos-slider").offset().top - 140 },
        "200"
      );

      $(".check-mark-slide-container").delay("350").slideToggle();
      var text = $(this).text();

      $(this).text(
        text == "ALLE PAKETDETAILS ANSEHEN"
          ? "WENIGER PAKETDETAILS ANSEHEN"
          : "ALLE PAKETDETAILS ANSEHEN"
      );
    });

    //first footer accordion openend
    if ($(window).width() < 768) {
      $(".widget_nav_menu.accordion:first")
    ...
    $(".check-mark-wrapper.clickable").on("click", function () {
      ...
      if (
        $(this)
          .find('.check-mark-tooltip[data-tooltiptext="' + index + '"]')
          .hasClass("active")
      ) {
        $(this)
          .find('.check-mark-tooltip[data-tooltiptext="' + index + '"]')
        ...
        $(".check-mark-wrapper.clickable")
          .not(this)
          .find(".check-mark-tooltip")
          .removeClass("active");
        $(".check-mark-wrapper")
          .not(this)
          .find(".check-mark:first")
          .removeClass("checked");
      }
    ...
    $(".nav-mobile ul.nav-category li .nav_toggle_wrapper").on(
      "click",
      function (e) {
        e.preventDefault();
        ...
        if ($(this).children("img").hasClass("active")) {
          ...
          $(".nav-mobile ul.nav-category li .nav_toggle_wrapper")
            .parents("li")
            .removeClass("active");
    ...
    */
    ...
    if ($(".mixitup_block_contents").length) {
      var initialFilter = $(".mixitup_block_titles li:first-child").data(
        "filter"
      );
    ...
    $(".display_more_top span, .display_more_top i").click(function () {
      ...
      var changetext =
        text > "mehr anzeigen" ? "mehr anzeigen" : "weniger anzeigen";
    ...
    $(".map_pin").click(function () {
      ...
      if (
        !$(".herold_map_single_result")
          .data("resultidctn", getID)
          .hasClass("active")
      ) {
    ...
    jQuery(document).ready(function ($) {
      ...
      jQuery(".cmpboxbtnyes").click(function () {
        fillUtmParameters();
      });

      function fillUtmParameters() {
        //save utm parameter to cookie and fill into gravity forms form
        const urlParams = new URLSearchParams(window.location.search);
        const utmSource = urlParams.get("utm_source");
        const utmMedium = urlParams.get("utm_medium");
        const utmCampaign = urlParams.get("utm_campaign");
        //const utmSource = getCookie('utmSource') ? getCookie('utmSource') : setCookie('utmSource', urlParams.get('utm_source'));
        //const utmMedium = getCookie('utmMedium') ? getCookie('utmMedium') : setCookie('utmMedium', urlParams.get('utm_medium'));
        //const utmCampaign = getCookie('utmCampaign') ? getCookie('utmCampaign') : setCookie('utmCampaign', urlParams.get('utm_campaign'));

        $("#gform_2 .utmsource").find("input").val(utmSource);
        $("#gform_2 .utmmedium").find("input").val(utmMedium);
        $("#gform_2 .utmcampaign").find("input").val(utmCampaign);
      }
    ...
    $(document).ready(function () {
      ...
      function referenzenCardSlider() {
        $(".referenzen .card-slider").slick({
          slidesToShow: 1,
          slidesToScroll: 1,
          infinite: true,
          autoplay: true,
          ...
          centerMode: false,
          variableWidth: false,
          prevArrow: $(".arrows_opacity").children(
            ".referenzen_prev_arrow_abs"
          ),
          nextArrow: $(".arrows_opacity").children(
            ".referenzen_next_arrow_abs"
          ),
          autoplaySpeed: 4000,
          appendDots: $(".arrows_referenzen_between"),
          mobileFirst: true,
          responsive: [
            {
              breakpoint: 769,
